...
for c in crange:
    ...
    kap_pure = kappa[0]*(1-c) + kappa[len(kappa)-1]*(c)
    u = (natoms*prefix*gamma*kappa[0])**(1/2) #Incorrect multiplication of the number of atoms in the unit cell
    uPUC = (prefix*gammaPUC*kap_pure)**(1/2)
    uPUC2 = (prefix*(gammaPUC2)*kap_pure)**(1/2)
    uPUCvac = (prefix*(gammaVac)*kap_pure)**(1/2)
    if i==0:
        kL[i] =kappa[0]
        kL_PUC[i] = kappa[0]
        kL_PUC2[i] = kappa[0]
        ...
        kL_PUC[i] = kap_pure*np.arctan(uPUC)/uPUC
        kL_PUC2[i] = kap_pure*np.arctan(uPUC2)/uPUC2
        kL_vac[i] = kap_pure*np.arctan(uPUCvac)/uPUCvac
    i=i+1
